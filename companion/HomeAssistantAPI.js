...
HomeAssistantAPI.prototype.fetchEntity = function(entity) {
    let self = this;
    if (self.isValid()) {
        //DEBUG console.log(`SENT ${self.address()}/api/states/${entity}`);
...
HomeAssistantAPI.prototype.fetchApiStatus = function() {
    let self = this;
    if (self.isValid()) {
        //DEBUG console.log(`SENT ${self.address()}/api/config`);
 ...
 * Change entity
 * @param {string} entity - Entity name
 * @param {string} service - Service to call for an entity
 * @param {string} state - Forced state value
 */
HomeAssistantAPI.prototype.changeEntity = function(entity, service, state) {
    let self = this;
    if (self.isValid()) {
        const json = JSON.stringify({
            entity_id: `${entity}`
        ...
        service = ServiceOverrides[domain] || service;
        //DEBUG console.log(`SENT ${self.address()}/api/services/${group}/${service} FOR ${entity}`);
        fetch(`${self.address()}/api/services/${group}/${service}`, {
            method: "POST",
            body: json,
            headers: {
                "Authorization": `Bearer ${self.token}`,
